...
DEFAULT_OBJECT_SEG_MODEL = "facebook/sam-vit-large"
...
MIN_OBJECT_WIDTH_FRAC = 0.04
DEFAULT_OBJECT_WIDTH_FRAC = 0.085
MAX_OBJECT_WIDTH_FRAC = 0.10
...
def _build_sam_pipeline(sam_model_name: str):
    ...
    try:
        ...
        raise RuntimeError(
            "SAM object cutout requires transformers with SamModel/SamProcessor support. Install/upgrade transformers."
        ) from exc

    _SAM_PROCESSOR = SamProcessor.from_pretrained(sam_model_name)
    _SAM_MODEL = SamModel.from_pretrained(sam_model_name)
...
def _split_input_list(values: list[str]) -> list[str]:
    items: list[str] = []
    for value in values:
        if value is None:
            continue
...
def _make_combo_output_path(base_output: str, scene_path: str, object_path: str) -> str:
    ...
    object_stem = Path(object_path).stem
    combo_dir = output_dir / f"{output_path.stem}__{scene_stem}__{object_stem}"
    return str(combo_dir / output_path.name)


def _copy_debug_run_to_combo_dir(debug_run: DebugRun, combo_output_path: str) -> Path:
    combo_output = Path(combo_output_path)
    combo_dir = combo_output.parent
    combo_dir.mkdir(parents=True, exist_ok=True)
    if debug_run.run_dir.resolve() != combo_dir.resolve():
        for child in debug_run.run_dir.iterdir():
            target = combo_dir / child.name
            if child.is_dir():
                if target.exists():
        ...
        shutil.rmtree(debug_run.run_dir)
    return combo_dir
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--scene", nargs="+", required=True, help="Path to the scene image, or multiple paths separated by spaces and/or commas")
    parser.add_argument("--object-image", nargs="+", required=True, help="Path to the product/object image, or multiple paths separated by spaces and/or commas")
    parser.add_argument("--object-label", required=True, nargs="+", help="One or more human labels for the object images, e.g. 'ginger ale can'. Provide one label for all objects or one label per object image.")
                ...
                for ny in range(max(0, cy - 1), min(h, cy + 2)):
                    for nx in range(max(0, cx - 1), min(w, cx + 2)):
                        if binary[ny, nx] and not visited[ny, nx]:
                            visited[ny, nx] = True
                            stack.append((ny, nx))
            if touches_border and len(coords) < int(h * w * 0.03):
                continue
...
def _sam_box_for_object(size: tuple[int, int]) -> list[list[int]]:
    w, h = size
    x0 = int(round(w * 0.08))
    y0 = int(round(h * 0.04))
    x1 = int(round(w * 0.92))
...
def _extract_mask_with_sam(obj_image: Image.Image, sam_model_name: str) -> Image.Image:
    if torch is None:
        raise RuntimeError("SAM object cutout requires torch.")
    processor, model = _build_sam_pipeline(sam_model_name)
    image_rgb = obj_image.convert("RGB")
    input_boxes = [_sam_box_for_object(image_rgb.size)]
    inputs = processor(images=image_rgb, input_boxes=input_boxes, return_tensors="pt")
    if torch.cuda.is_available():
    ...
    mask_batch = masks[0][0].detach().float().cpu().numpy()

    flat_iou_scores = np.asarray(iou_scores, dtype=np.float32).reshape(-1)

    h, w = image_rgb.size[1], image_rgb.size[0]
    center_x = (w - 1) / 2.0
    center_y = (h - 1) / 2.0
    border_margin_x = max(1, int(w * 0.03))
    border_margin_y = max(1, int(h * 0.03))

    best_mask = None
    best_score = -1e18
    for idx, raw_mask in enumerate(mask_batch):
        raw_mask = np.asarray(raw_mask, dtype=np.float32)
        binary = raw_mask > 0.0
        if not binary.any():
            continue
        ...
        touches_border = (
            x0 <= border_margin_x or y0 <= border_margin_y or
            x1 >= w - border_margin_x or y1 >= h - border_margin_y
        )
        cx = float(xs.mean())
        cy = float(ys.mean())
        center_dist = ((cx - center_x) / max(1.0, w * 0.5)) ** 2 + ((cy - center_y) / max(1.0, h * 0.5)) ** 2
        bbox_fill = float(binary[y0:y1, x0:x1].mean()) if (x1 > x0 and y1 > y0) else 0.0
        sam_iou = float(flat_iou_scores[idx]) if idx < len(flat_iou_scores) else 0.0
        score = sam_iou * 4.0
        score += bbox_fill * 1.25
        score -= abs(area_frac - 0.22) * 2.2
    ...
    if best_mask is None:
        raise RuntimeError("SAM could not extract a usable object mask.")

    mask_img = Image.fromarray((best_mask.astype(np.uint8) * 255), mode="L")
    mask_img = _erode_mask(mask_img, 1)
    mask_img = _dilate_mask(mask_img, 2)
    mask_img = mask_img.filter(ImageFilter.GaussianBlur(radius=1.1))
    return mask_img
...
def extract_object_rgba(obj_image: Image.Image, client: InferenceClient, seg_model: str) -> Image.Image:
    ...
    alpha_nonzero = ImageStatProxy.nonzero_fraction(alpha)
    if bbox and 0.01 < alpha_nonzero < 0.99:
        return obj_image.crop(bbox)

    mask = _extract_mask_with_sam(obj_image, seg_model)
    if mask.size != obj_image.size:
        mask = mask.resize(obj_image.size, Image.LANCZOS)

    mask_nonzero = ImageStatProxy.nonzero_fraction(mask)
    if mask_nonzero <= 0.001 or mask_nonzero >= 0.98:
        raise RuntimeError("SAM extracted an unusable object mask.")
...
def run_once(args: argparse.Namespace) -> dict:
    ...
    return {
        "debug_run": debug_run,
        "final_output": debug_run.final_output,
        "run_dir": debug_run.run_dir,
    }
...
def _stem_slug(path_str: str) -> str:
    stem = Path(path_str).stem or "item"
    stem = re.sub(r"[^A-Za-z0-9._-]+", "_", stem).strip("._-")
    return stem or "item"


def _resolve_object_labels(object_images: list[str], object_labels: list[str]) -> dict[str, str]:
    labels = _split_cli_values(object_labels)
    if len(labels) == 1:
        return {obj_path: labels[0] for obj_path in object_images}
    if len(labels) != len(object_images):
        raise ValueError(
            f"Expected either 1 --object-label for all objects or {len(object_images)} labels (one per object image), got {len(labels)}."
        )
    return {obj_path: label for obj_path, label in zip(object_images, labels)}
...
def main() -> int:
    ...
    scene_paths = _split_cli_values(args.scene)
    object_paths = _split_cli_values(args.object_image)
    if not scene_paths:
        raise ValueError("At least one --scene path is required.")
    if not object_paths:
        raise ValueError("At least one --object-image path is required.")

    object_label_map = _resolve_object_labels(object_paths, args.object_label)
    multi_run = len(scene_paths) * len(object_paths) > 1
    client = build_client(args.provider)
    ...
    for scene_path in scene_paths:
        for object_path in object_paths:
            ...
            print(f"[{run_index}/{total_runs}] Scene: {run_args.scene} | Object: {run_args.object_image} | Label: {run_args.object_label}")
            print(f"Run directory: {debug_run.run_dir}")
            print("[1/6] Extracting object matte...")
            obj_rgba = extract_object_rgba(obj, client, run_args.object_seg_model)
            object_mask = obj_rgba.getchannel("A")
            debug_run.save_image(scene_full, "01_scene_original.png", mode="RGB")
            debug_run.save_image(scene_for_seg, "02_scene_for_segmentation.png", mode="RGB")
            debug_run.save_image(obj, "03_object_input.png")
            ...
            candidates_small, person_mask_small, forbidden_mask_small = find_support_candidates(
                scene_for_seg.convert("RGB"),
                depth_small,
                depth_pref=run_args.object_scene_depth,
                client=client,
                object_label=run_args.object_label,
                scene_seg_model=run_args.scene_seg_model,
            )
            candidates_full: list[SupportCandidate] = []
            person_mask_full = person_mask_small.resize(scene_full.size, Image.NEAREST)
            forbidden_mask_full = forbidden_mask_small.resize(scene_full.size, Image.NEAREST)
            ...
            print("[3/6] Computing systematic placement and full-resolution precompositing...")
            placement = compute_auto_placement(scene_full, obj_rgba, chosen_candidate, run_args)
            placement_preview = draw_placement_preview(scene_full, placement, chosen_candidate)
            precomp_full, resized_obj, shadow_full, object_layer_full = precompose(scene_full, obj_rgba, placement)
            debug_run.save_image(placement_preview, "10_placement_preview_fullres.png")
            debug_run.save_image(resized_obj, "11_object_resized.png")
            ...
            prompt = run_args.prompt.strip() if run_args.prompt and run_args.prompt.strip() else default_prompt(run_args.object_label, placement.support_label)
            crop_box = compute_local_crop_box(placement, scene_full.size)
            crop_x0, crop_y0, crop_x1, crop_y1 = crop_box
            original_crop = scene_full.crop(crop_box)
            precomp_crop = precomp_full.crop(crop_box)
            ...
            if run_args.skip_refine:
                print("[4/6] Skipping HF refinement as requested.")
                refined_crop_full = precomp_crop.convert("RGB")
            else:
                print("[4/6] Refining only the local crop with HF image edit model...")
                print(f"      Model: {run_args.edit_model}")
                refined_crop_small = refine_with_hf(precomp_crop_for_edit, client, prompt, run_args.edit_model).convert("RGB")
                debug_run.save_image(refined_crop_small, "19_refined_local_crop_model_output.png", mode="RGB")
                if refined_crop_small.size != precomp_crop.size:
                    refined_crop_full = refined_crop_small.resize(precomp_crop.size, Image.LANCZOS)
                else:
            ...
            metadata = {
                "scene": str(Path(run_args.scene).resolve()),
                "object_image": str(Path(run_args.object_image).resolve()),
                "requested_output": str(Path(run_args.output).resolve()),
                "final_output": str(debug_run.final_output.resolve()),
                "run_dir": str(debug_run.run_dir.resolve()),
                "prompt": prompt,
                "provider": run_args.provider,
                "edit_model": run_args.edit_model,
                "object_seg_model": run_args.object_seg_model,
                "scene_seg_model": run_args.scene_seg_model,
                "depth_model": DEFAULT_DEPTH_MODEL,
                "support_model": DEFAULT_AFFORDANCE_MODEL,
                "placement": asdict(placement),
                ...
                "edit_input_size": {"width": precomp_crop_for_edit.width, "height": precomp_crop_for_edit.height},
                "crop_scale_sent_to_model": crop_scale,
                "skip_refine": run_args.skip_refine,
                "object_scene_depth": run_args.object_scene_depth,
                "internal_policy": {
                    "depth_model": DEFAULT_DEPTH_MODEL,
                    "support_model": DEFAULT_AFFORDANCE_MODEL,
                    "segmentation_max_side": INTERNAL_SEG_MAX_SCENE_SIDE,
