...
def build_scene_occupancy_masks(
    ...
    try:
        seg_outputs = client.image_segmentation(
            pil_to_png_bytes(scene_rgb.convert("RGB")),
            model=scene_seg_model,
            content_type="image/png",
        )

    except Exception as exc:
        print(f"[masking] scene segmentation failed: {exc}")
        return empty.copy(), empty.copy()
...
def _heatmap_to_candidates(
    ...
    # Remove forbidden occupancy directly from candidate generation.
